...
def heart_tracks
  ...
  execute(<<-SQL)
  SELECT title, COUNT(tracks.*) as heart_tracks
  FROM albums
  JOIN tracks ON albums.asin = tracks.album
  WHERE song LIKE '%Heart%'
  GROUP BY title
  ORDER BY heart_tracks DESC, title
  SQL
end
...
def top_track_counts
  ...
  execute(<<-SQL)
  SELECT albums.title, COUNT(tracks.*) as num_tracks
  FROM albums
  JOIN tracks ON albums.asin = tracks.album
  GROUP BY albums.asin
  ORDER BY num_tracks DESC, title DESC
  LIMIT 10
  SQL
end
